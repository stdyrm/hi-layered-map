...
const convertCoordinatesLineString: IConvertCoordinates = (
	geometry,
	epsgFrom,
	epsgTo
) => {
...
const convertCoordinatesPolygon: IConvertCoordinates = (
	geometry,
	epsgFrom,
	epsgTo
) => {
...
const convertCoordinatesMultiPolygon: IConvertCoordinates = (
	geometry,
	epsgFrom,
	epsgTo
) => {
...
const convertEpsgType: IConvertEpsg = async (geojson, epsgFrom, epsgTo) => {
	...
	geojson.features.map((feat) => {
		...
		if (feat.geometry.type === "LineString") {
			feat.geometry.coordinates = convertCoordinatesLineString(
				feat.geometry,
				epsg[epsgFrom],
				epsg[epsgTo]
			);
		} else if (feat.geometry.type === "Polygon") {
			feat.geometry.coordinates = convertCoordinatesPolygon(
				feat.geometry,
				epsg[epsgFrom],
				epsg[epsgTo]
			);
		} else if (feat.geometry.type === "MultiPolygon") {
			feat.geometry.coordinates = convertCoordinatesMultiPolygon(
				feat.geometry,
				epsg[epsgFrom],
				epsg[epsgTo]
			);
			...
			throw new Error(
				`GeoJSON file has an unknown geometry type: ${feat.geometry.type}`
			);
		}
	});
